...
fn test_concatenate_flat() {
	...
	expected := nd.from_int_1d([1, 1, 1, 0, 0])
...
fn test_hstack() {
	...
	b := num.zeros([2, 2])
	result := num.hstack([a, b])
	expected := nd.from_int([1, 1, 0, 0, 1, 1, 0, 0], [2, 4])
	assert (nd.allclose(result, expected))
}
...
fn test_dstack_flat() {
	...
	expected := nd.from_int([1, 0, 1, 0, 1, 0], [1, 3, 2])
	assert (nd.allclose(result, expected))
}
...
fn test_dstack() {
	...
	b := num.zeros([2, 2])
	result := num.dstack([a, b])
	expected := nd.from_int([1, 0, 1, 0, 1, 0, 1, 0], [2, 2, 2])
	assert (nd.allclose(result, expected))
}
...
fn test_column_stack_flat() {
	...
	expected := nd.from_int([1, 0, 1, 0], [2, 2])
	assert (nd.allclose(result, expected))
}
...
fn test_stack() {
	...
	expected := nd.from_int([1, 1, 0, 0, 1, 1, 0, 0], [2, 2, 2])
	assert (nd.allclose(result, expected))
}
